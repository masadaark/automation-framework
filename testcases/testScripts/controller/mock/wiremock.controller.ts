...
export class WiremockController {
  ...
  public async POST(file: string) {
    ...
    const promises = mappings.map(async (mapping) => {
      const apiPath = mapping?.paramReplace
        ? VFormatter.PathReplace(wiremockFile.apiPath, VFormatter.Exec(mapping.paramReplace))
        : VFormatter.Exec(wiremockFile.apiPath);
