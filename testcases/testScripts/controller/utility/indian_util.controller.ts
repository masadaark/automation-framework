...
export class UtilityController {
    ...
    public UniqueArrayVar(varName: string): void {
        ...
        const uniqueArray = Array.isArray(array) && array.length > 0
            ? (typeof array[0] === 'object' ? _.uniqWith(array, _.isEqual) : _.uniq(array))
            : array;
    ...
    public VarArrToQueryParam(arrVarName: string, queryKey: string, varName: string): void {
        const array = StorageClass.get(arrVarName).map((v: string) => `${queryKey}=${v}`).join("&")
