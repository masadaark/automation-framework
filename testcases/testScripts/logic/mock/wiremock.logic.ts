...
export default class WiremockLogic {
  ...
  static async POST(i: {
    ...
    const httpResponse = await ProtocolHttp.REQUEST(this._wiremockURL, 'POST', {}, requestWiremock);
  ...
  private static BuildRequest(
    ...
    if (/query/i.test(paramType) || (/get/i.test(method) && Validator.Var(req))) {
      ...
      if (Validator.Var(req)) {
        reqObj.bodyPatterns =
          headers['content-type'] === 'application/xml' && typeof req === 'string'
            ? [{ equalToXml: req }]
  ...
  static readonly ClearMappingUUIDs = () => {
    for (const uuid of this._uuids) ProtocolHttp.REQUEST(this._wiremockURL + uuid, 'DELETE');
    this._uuids = [];
  };
